...
|-----------------------|---------------------------------|----------|
| `Catalog:changed`     | `IProduct[]`                   | Генерируется при обновлении списка товаров |
| `Catalog:loading`     | `boolean`                      | Статус загрузки данных |

### Корзина
| Событие               | Данные                          | Описание |
|-----------------------|---------------------------------|----------|
| `Basket:add`          | `{ productId: string }`        | Добавление товара в корзину |
| `Basket:remove`       | `{ productId: string }`        | Удаление товара из корзины |
| `Basket:update`       | `Map<string, number>`          | Обновление состояния корзины |
| `Basket:clear`        | `-`                            | Очистка корзины |

### Модальные окна
| Событие               | Данные                          | Описание |
|-----------------------|---------------------------------|----------|
| `Modal:open`          | `{ modalType: string }`        | Открытие модального окна |
| `Modal:close`         | `{ modalType: string }`        | Закрытие модального окна |

### Форма заказа
| Событие               | Данные                          | Описание |
|-----------------------|---------------------------------|----------|
| `OrderForm:submit`    | `IOrder`                       | Отправка формы заказа |
| `OrderForm:changed`   | `Partial<IOrder>`              | Изменение данных формы |
| `OrderForm:validated` | `{ isValid: boolean }`         | Результат валидации |
